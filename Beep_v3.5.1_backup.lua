...
local GameProfiles = {
    ["Universal"] = {part = "Head", visible = false, prediction = 0,  fireRate = 0.03, fireMethod = "auto"},
    ["Rivals"]    = {part = "Head", visible = true,  prediction = 0,  fireRate = 0.0,  fireMethod = "hold"},
    ["Arsenal"]   = {part = "Head", visible = false, prediction = 0,  fireRate = 0.0,  fireMethod = "hold"},
    ["Jailbreak"] = {part = "Head", visible = true,  prediction = 2,  fireRate = 0.10, fireMethod = "tool"},
}
    ...
    if Config.Combat.RagebotAutoTP then
        ...
        if myRoot then
            ...
            if Config.Combat.RagebotTPMode == "Teleport" then
                -- Instant teleport next to the target
                myRoot.CFrame = CFrame.new(goalPos, target.Position)
                pcall(function() myRoot.AssemblyLinearVelocity = Vector3.new(0, 0, 0) end)
            else
                -- Fly: move toward the target at high speed
                local toGoal = (goalPos - myRoot.Position)
                local dist = toGoal.Magnitude
                if dist > 0.5 then
                    local step = math.min(Config.Combat.RagebotTPSpeed * dt, dist)
                    myRoot.CFrame = CFrame.new(myRoot.Position + (toGoal.Unit * step), target.Position)
                    pcall(function() myRoot.AssemblyLinearVelocity = Vector3.new(0, 0, 0) end)
                end
            end
        end
