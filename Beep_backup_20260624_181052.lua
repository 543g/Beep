...
local function ParseHexColor(hexStr)
    -- Remove # and spaces, convert to uppercase
    local hex = hexStr:gsub("#", ""):gsub(" ", ""):upper()
    
    -- Support short format (FFF -> FFFFFF)
    if #hex == 3 then
...
function UI:CreateColorInput(parent, text, configSection, configKey, callback)
    ...
    HexInput.FocusLost:Connect(function()
        local newColor, cleanHex = ParseHexColor(HexInput.Text)
        if newColor and cleanHex then
            Config[configSection][configKey] = newColor
            ColorPreview.BackgroundColor3 = newColor
            HexInput.Text = cleanHex
    ...
    -- Update preview when accent changes (if this is for accent)
